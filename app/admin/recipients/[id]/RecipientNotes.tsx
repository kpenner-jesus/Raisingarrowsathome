...
export function RecipientNotes({ recipientId, currentUserId, notes }: {
  recipientId: string;
  ...
  async function add() {
    ...
    try {
      const r = await fetch(`/api/admin/recipients/${recipientId}/notes`, {
        method: "POST",
  ...
  return (
    <section className="ra-card">
      <h3 className="ra-section-title">Internal notes (recipient)</h3>
      <p className="ra-quiet" style={{ marginTop: 0, fontSize: "0.85rem" }}>
        Cross-year context for this family. Visible only to admins.
      </p>
