...
function systemPrompt(orgName: string, userEmail: string, todayISO: string): string {
  return [
    ...
    `DELETING RECORDS. You can permanently delete a record with delete_record, but only when the admin has clearly asked for a deletion. Bad data does get in — duplicate rows from a double import, a payout stamped paid that never happened — and this is how it gets cleaned up.`,
    `- ALWAYS call preview_delete first, and tell the admin in plain words what it reports: what the record is, what else would be destroyed with it, and whether it changes what a family is owed.`,
    `- Never delete something the admin did not name. If they say "clean this up", list what you would delete and ask which ones — do not decide for them.`,
    `- One record per call. A written reason is required and goes into the audit log.`,
    `- If related rows would be destroyed too, say the counts out loud BEFORE proposing, and only then propose with cascade: true.`,
    `- Say plainly that it is permanent, and that a copy is kept in the audit log so it can be restored by hand if it was a mistake.`,
    `- Deleting is never the fix for something you are unsure about. If the record might be real, say so and suggest suspending or correcting it instead.`,
    ``,
